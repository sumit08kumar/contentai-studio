...
def worker_node(state: dict) -> dict:
    ...
    section_md = response.content if isinstance(response.content, str) else str(response.content)
...
def _sdxl_generate_image_bytes(prompt: str) -> bytes:
    """
    Returns raw image bytes generated by Stability AI SDXL.
    ...
    # image is a PIL.Image object; convert to PNG bytes
    import io
    buf = io.BytesIO()
...
def generate_and_place_images(state: State) -> dict:
    ...
    for spec in image_specs:
        ...
        if not out_path.exists():
            try:
                img_bytes = _sdxl_generate_image_bytes(spec["prompt"])
...
g.add_node("worker", worker_node)  # type: ignore[arg-type]
